...
export type { GscTopQueriesData, GscTopQueryRow }

export interface TopQueriesTableProps {
    initialDays?: number
    limit?: number
    perPage?: number
    siteUrl?: string | null
...
export function TopQueriesTable(props: TopQueriesTableProps): JSX.Element {
    const { initialDays = 28, limit = 25, perPage = 10, siteUrl, baseUrl, fetchImpl } = props

    const [days, setDays] = useState<number>(initialDays)
    const [data, setData] = useState<GscTopQueriesData | null>(null)
    ...
    if (errorCode === 'base_not_installed') {
        return (
            <div className="ap-gsc-top-queries__missing-base" role="alert">
                <p>{errorMessage}</p>
                <pre>
    ...
    return (
        <div className="ap-gsc-top-queries">
            <header className="ap-gsc-top-queries__header">
                <h2 className="ap-gsc-top-queries__title">Top queries</h2>
                <label className="ap-gsc-top-queries__range">
                    <span className="ap-gsc-top-queries__range-label">Range</span>
                    <select
                        value={days}
            ...
            {errorMessage ? (
                <div className="ap-gsc-top-queries__error" role="alert">
                    {errorMessage}
                </div>
            ) : visibleRows.length > 0 ? (
                <>
                    <table className="ap-gsc-top-queries__table">
                        <thead>
                            <tr>
                                {COLUMNS.map((col) => (
                                    <th key={col.key} scope="col">
                                        <button
                                            type="button"
                                            className="ap-gsc-top-queries__sort"
                    ...
                    <nav className="ap-gsc-top-queries__pagination" aria-label="Top queries pagination">
                ...
                <p className="ap-gsc-top-queries__empty">No queries for this range yet.</p>
            )}
        </div>
    )
}
