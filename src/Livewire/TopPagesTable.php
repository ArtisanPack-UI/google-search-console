...
/**
 * Search Console top pages table Livewire component.
 *
...
use ArtisanPackUI\GoogleSearchConsole\Reporting\TopPagesData;
...
/**
 * Renders the Search Console top pages table with server-fetched rows
 * and client-side sort + paginate.
 *
 * @package    ArtisanPack_UI
 * @subpackage GoogleSearchConsole
 *
 * @since 1.0.0
 */
class TopPagesTable extends Component
{
    public int $days = 28;

    public int $limit = TopPagesFetcher::DEFAULT_LIMIT;
    ...
    /**
     * @var list<array{page: string, clicks: float, impressions: float, ctr: float, position: float}>
     */
    public array $rows = [];
    ...
    public function mount( int $days = 28, int $limit = TopPagesFetcher::DEFAULT_LIMIT, ?string $siteUrl = null ): void
    {
    ...
    {
        $allowed = [ 'page', 'clicks', 'impressions', 'ctr', 'position' ];
        ...
        if ( $this->sortBy === $column ) {
            ...
            $this->sortBy  = $column;
            $this->sortDir = 'position' === $column || 'page' === $column ? 'asc' : 'desc';
        }
    ...
    {
        return view( 'google-search-console::livewire.top-pages-table', [
    ...
    protected function applyData( TopPagesData $data ): void
    {
        $this->rows = $data->rows;
    }
}
